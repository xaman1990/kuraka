...
| 2026-05-12 | **Skills decoupling**: 23 skills refactorizados, `kuraka.md`/`kuraka-modes.md`/`kuraka-policies.md`/`gap-analysis.md` traducidos a inglés. 0 sie_v2 mentions en `skills/`. | v0.3.1 |
| 2026-05-13 | **Project-layer rules migration** + `mount-kuraka.sh` fix: 13 rules de sie_v2 (01-15 menos 08+10 redundantes) copiados a `migration-examples/sie_v2-project-layer/conventions/` para que los agentes los lean al mountar. README del project-layer actualizado con lista completa, mapeo regla→agente, y secuencia de adopción documentada con `export KURAKA_VAULT`. `mount-kuraka.sh`: bug fix `cp -u` (no soportado en BSD/macOS) + pre-flight check que detecta ausencia de `kuraka.config.yaml` o `.claude/project/` y muestra cómo completarlas. | pending |
